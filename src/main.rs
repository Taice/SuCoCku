mod settings;
use std::{collections::LinkedList, f32};
...
async fn main() {
    ...
    let font = load_ttf_font_from_bytes(include_bytes!("../assets/Roboto-Regular.ttf")).unwrap();
    loop {
        clear_background(GRAY);
        sudoku.draw(&settings, &font);
        next_frame().await;
    }
}
...
impl Default for Sudoku {
    fn default() -> Self {
        Self {
            nums: [[0b111111111; 9]; 9],
        }
    }
}

impl Sudoku {
    pub fn draw(&self, s: &Settings, font: &Font) {
        ...
        self.draw_grid(s, box_size, font);
    }

    fn draw_grid(&self, s: &Settings, box_size: f32, font: &Font) {
        let text_params = TextParams {
            font: Some(font),
            font_size: s.get_num_font_size(box_size),
            font_scale: s.num_font_scale,
            color: s.font_color,
            ..Default::default()
        };
        ...
        for (i, row) in self.nums.iter().enumerate() {
            ...
            for (j, n) in row.iter().enumerate() {
                ...
                if n & (1 << 15) == 0 {
                    ...
                    draw_notes(s, box_size, x, note_y, *n, font);
                // num
                } else {
